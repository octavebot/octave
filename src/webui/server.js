...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && pbMatch) {
      ...
      // inline so the dashboard PDF button opens the doc in a new tab
      // (target="_blank") rather than forcing a download. Cache briefly —
      // PDFs only regenerate when backtest stats refresh.
      res.setHeader('content-disposition', `inline; filename="${pbMatch[1]}.pdf"`);
